...
export function Layout(props: Props) {
  ...
  const getJustify = (): SerializedStyles => {
    const term = getTerm() ?? 'start';
    return css`
      justify-content: ${term};
      justify-items: ${term};
    `;
  };

  const getAlign = (): SerializedStyles => {
    const term = getTerm() ?? 'stretch';
    return css`
      align-items: ${term};
      align-content: ${term};
    `;
...
//     const flexBasisValue = Number.isFinite(flexBasis)
//       ? `${flexBasis}px`
//       : flexBasis;
